package com.as3dmod.modifiers {
	...
	public class Skew extends Modifier implements IModifier {
		 ...
		 */
		public function Skew(f:Number=0) { _force = f; }
		...
		public function set offset(o:Number):void { _offset = XMath.trim(0, 1, o); }
		
		/** Мощность воздействия модификатора. */
		public function get power():Number { return _power; }
		public function set power(power:Number):void { _power = Math.max(1, power); }
		
		/** Определяет участок меша на который модификатор не действует. */
		public function get falloff():Number { return _falloff; }
		public function set falloff(falloff:Number):void { _falloff = XMath.trim(0, 1, falloff); }
		
		/** Если true, то модификатор наклоняет только стороны меша. */
		public function get oneSide():Boolean { return _oneSide; }
		public function set oneSide(oneSide:Boolean):void { _oneSide = oneSide; }
		
		/** Оси, вдоль которых выполняется наклон меша. */
		public function get skewAxis():int { return _skewAxis; }
		public function set skewAxis(skewAxis:int):void { _skewAxis = skewAxis; }
		
		/** Меняет местами оси, вдоль которых выполняется наклон меша. */
		public function get swapAxes():Boolean { return _swapAxes; }
		public function set swapAxes(spawAxes:Boolean):void { _swapAxes = spawAxes; }
		
		/** Переносит участок на который модификатор не действует на противоположную сторону меша. */
		public function get inverseFalloff():Boolean { return _inverseFalloff; }
		public function set inverseFalloff(inverseFalloff:Boolean):void { _inverseFalloff = inverseFalloff; }
